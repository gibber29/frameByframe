...
function Result({ game, go }: { game: BadlyState; go: (path: string) => void }) {
  ...
  const shareResult = async () => { try { if (navigator.share) await navigator.share({ text: share }); else await navigator.clipboard.writeText(share); setCopied(true); window.setTimeout(() => setCopied(false), 1800) } catch { /* native share cancelled */ } }
